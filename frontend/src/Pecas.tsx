  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingPeca) {
      try {
        ...
        carregarPeças(); // Recarrega os dados reais do banco
      ...
      try {
        // Remove o ID temporário (0) para que o Prisma gere o ID real (autoincrement)
        const { id, ...pecaSemId } = currentPeca;

        const response = await fetch("http://localhost:3000/peca", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(pecaSemId),
        });

        if (response.ok) {
          alert("Peça cadastrada com sucesso!");
          carregarPeças(); // Busca a lista atualizada do banco de dados com os IDs corretos
        } else {
          const data = await response.json();
